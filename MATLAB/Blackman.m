...
blackman = create_blackman_window(N);

% Plot Blackman window and ideal low-pass filter
plot_window_and_ideal_filter(blackman, cutoff_frequency, fs);

% Create Low Pass Filter using Blackman window
lpf = create_lpf(blackman, cutoff_frequency, fs);

% Plot Blackman window frequency response
plot_frequency_response(blackman, fs);
...
function blackman_window = create_blackman_window(N)
    n = 0:N-1;
...
function lpf = create_lpf(blackman_window, cutoff_frequency, fs)
    ideal_lpf_response = ones(size(blackman_window));
    ideal_lpf_response(round(cutoff_frequency * length(blackman_window) / fs)+1:end) = 0;

    lpf = conv(blackman_window, ideal_lpf_response, 'same');
    lpf = lpf / sum(lpf);
end
